...
class MultiScaleImageFlow(ImageFlow):
    """
    uses splits to generate different levels of detail
    """
    def __init__(self):
        super().__init__(MultiScaleImageFlow.create_layer_list())

    def create_layer_list():
        flow_layers = []
        vardeq_layers = [CouplingLayer(network=GatedConvNet(c_out=2, c_hidden=16),
                                    mask=FlowMasks.create_checkerboard_mask(h=28, w=28, invert=(i%2==1)),
        ...
        for i in range(2):
            flow_layers += [CouplingLayer(network=GatedConvNet(c_out=8, c_hidden=48),
                                                    mask=FlowMasks.create_channel_mask(c_in=4, invert=(i % 2 == 1)),
                                                    c_in=4)]
        flow_layers += [SplitFlow(),
                        SqueezeFlow()]
        for i in range(4):
            flow_layers += [CouplingLayer(network=GatedConvNet(c_out=16, c_hidden=64),
                                        mask=FlowMasks.create_channel_mask(c_in=8, invert=(i%2==1)),
                                        c_in=8)]
        # flow_layers = []
