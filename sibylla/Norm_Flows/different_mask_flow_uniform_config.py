...
def make_flow_model(event_shape: Sequence[int],
    ...
    alternating_mask = jnp.arange(0, np.prod(event_shape)) % 2
    alternating_mask = jnp.reshape(alternating_mask, event_shape)
    alternating_mask = alternating_mask.astype(bool)

    # maxval needs to be 2 to get numbers in 0,1
    random_mask = jax.random.randint(jax.random.PRNGKey(1), alternating_mask.shape, minval=0, maxval=2).astype(bool)
    ...
    layers = []
    layer_index = 0
    while layer_index < num_layers//2:
        ...
        alternating_mask = jnp.logical_not(alternating_mask)
    ...
    while layer_index < num_layers:
        ...
        layers.append(layer)
        # Flip the mask after each layer.
        random_mask = jnp.logical_not(random_mask)
...
def get_config(dataset_name : str) -> config_dict.ConfigDict:
    ...
    config.model_name = "different_mask_uniform" + dataset_name
